...
const columnsRiskRegister = [
  ...
  },
  {
    ...
    options: {
      filter: true,
      setCellHeaderProps: value => ({ style: { width: "70%" } }),
      sort: false
    }
  },
 {
...
export default function DEI() {
  ...
  const section = async () => {
    fetch(process.env.PUBLIC_URL + `/test/${'hr_compliance'}.json`)
      .then(function (res) {
  ...
  const RiskRegister = async () => {
    let data_ = (sessionStorage.getItem("currentModelName") == "Credit Lending") ? "hr_risks" : (sessionStorage.getItem("currentModelName") == "Fraud Detection") ? "hr_risks" : "hr_risks";
  ...
  return (
    <>
      ...
      {_section && current_section && value == 0 && (<PageTitle  dropDownName="Regulation Name" data={_section} tabs2={["Regulations","Risk Register","HR Analytics"]} initialData={current_section} selectValue={(data) => { setCurrentSection(data) }} width={["0.15","0.35","0.65"]} tabValue={handleChange} tabSelection={0} />)}
      {/* {algoName && currentAlgo &&  && (<PageTitle  dropDownName="Mitigation Algorithm" data={algoName} tabs2={["Regulations","Risk Register","Report"]} initialData={currentAlgo} selectValue={(data) => { setCurrentAlgo(data) }} width={["0.15","0.35","0.65"]} tabValue={handleChange} tabSelection={(value == 1)?1:value} />)} */}
      {(value == 1 || value == 2 || value == 3) && (<PageTitle  title="DEI"  tabs2={["Regulations","Risk Register","HR Analytics"]} width={["0.3","0.45","0.7"]} tabValue={handleChange} tabSelection={value} />)}
