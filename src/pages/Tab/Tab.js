...
export default function TabView() {
  ...
  return (
    <>
      ...
      {algoName.length > 0 && currentAlgo&& (<PageTitle title="Bias"  data={algoName} width={["0.5","0.5"]} dropDownName="Mitigation Algorithm" initialData={currentAlgo} selectValue={(data)=>{setCurrentAlgo(data)}}/>)}
      ...
      <div >
        <Grid container spacing={6}>
            {
              customValue && Object.keys(customValue).length > 0 && customValue[currentAlgo] && Object.keys(customValue[currentAlgo]).map(metric => <MetricsGraph name={metric}  algo={currentAlgo} data={customValue[currentAlgo][metric]} dates={dateList} />)
            }
        </Grid>
      </div>
        <br />
