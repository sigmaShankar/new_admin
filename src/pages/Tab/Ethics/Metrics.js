...
function MetricsGraph(props) {
  ...
  const data = [];
  (sessionStorage.getItem("currentModelName") == "Credit Lending") ?  props.dates.map(date => data.push({ name: "as on " + "10/25/2023" })) : props.dates.map(date => data.push({ name: "as on " + "10/25/2023" }))
