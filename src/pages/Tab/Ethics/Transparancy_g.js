...
function TransparancyGraph(props) {
  const [mitigateChk, setMitigateChk] = useState(true);
  const data = [];

  props.dates.map(date => data.push({ name: "as on " + date }))
  var temp = {};
  ...
  // if (props.algo === "Original_Model") setMitigateChk(false)
  return (
      ...
      <CardHeader className="border-0">
          <Row className="align-items-center" >
            <div className="col" style={{display:"inline-flex"}}>
              <h4 className="mb-0"  style={{cursor:"help",textTransform: "capitalize" }}> <b  >{props.name}</b></h4>
              <span style={{marginLeft:"auto",float:"right",cursor:"help"}}><i   style={{cursor:"help","font-size":"20px"}}  class="fa fa-info-circle" aria-hidden="true"></i></span>
            </div>
...
export default TransparancyGraph;
